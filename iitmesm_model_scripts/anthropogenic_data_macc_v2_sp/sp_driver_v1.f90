...
PROGRAM MACv2SP
  ...
  INTEGER, PARAMETER :: nlev = 64, nmon = 12
  integer, parameter :: NLVLS = 63, NLATS = 96, NLONS = 192, NRECS = 12
  real,dimension(NLONS,NLATS,NLVLS,NRECS) :: readvar_dz
  real,dimension(NLONS,NLATS,NLVLS+1,NRECS) :: readvar_z
  real :: lats(NLATS), lons(NLONS), lvls_1(NLVLS), recs(NRECS), lvls_2(NLVLS+1)


  CHARACTER(len = 100)::file_name,str,file
  CHARACTER(len = 6):: yyyy
  INTEGER :: lam
  integer :: year
...
character(len =4) :: lambda_arg
  ...
  print *,"Test1"
  call readfromnc("allforcing_present_day_mh_t63_height_dz.nc","height_dz", readvar_dz,lons,lats, lvls_1, recs, NLONS, NLATS, NLVLS, NRECS)
  print *,"Test2"
  call readfromnc("allforcing_present_day_mh_t63_height.nc","height",readvar_z,lons, lats, lvls_2, recs, NLONS, NLATS, NLVLS+1, NRECS)
  call getarg(1, lambda_arg)
  call getarg(2, yyyy)
  read(lambda_arg,*)lambda
  read(yyyy,*)year
  print *,"lambda = ",lambda
  write(lam , '(2f12.7)') lambda
  write(str , '(I5)') lam
  ...
  ! 
  DO iyear = year,year
   file_name='gt_t_'//trim(lambda_arg)//'_'//trim(yyyy)//'.nc'
    DO m = 1,nmon
    ...
    !IF (iyear == 2005) THEN
      ...
      iret = iret + nf90_create(trim(file_name), NF90_CLOBBER, ncid)    
