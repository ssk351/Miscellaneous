...
def load_data():
		...
		fsikkimjastemp = "sikkim_jastemp_1900_2008.txt"
		dsikkimjastemp = np.genfromtxt(fsikkimjastemp, delimiter=",", dtype=float).flatten()
		fclimate = "pdo_1900_2008_mon.txt"
		dclimate = np.genfromtxt(fclimate, delimiter=",", dtype=float).flatten()
		...
		return dsikkimjastemp, dclimate
...
def climate_anomaly_y(x, t):
    ...
    d1 = dt.datetime(1900, 1, 1)
    #d2 = dt.datetime(1980, 12, 31)
    d2 = dt.datetime(2008, 12, 31)
...
def iirfilter(x, N=5, pass_freq=1 / 8):
    """
