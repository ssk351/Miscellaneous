...
! The equations follow the link below
! http://www.thermopedia.com/content/137/     
! http://nit.colorado.edu/atoc5560/week8.pdf 
! The basic idea in Mie Theory is to calculate the extinction, scattering and assymetry
! parameters for different size of the particles which is indirectly represented by x.
! which is also the size parameter

  ! Mie x and y values.
  dx = x
  y  = x * refrel
! One Mie code runs for a particular size parameter only
! Series terminated after nstop terms, i.e. the nstop is an integer term which
! is the times the main Mie loop will run i.e. each time do calculations
! of the electromagnetic wave falling at an angle of 180
! The number of times Mie loop will run is the number of terms in the Mie Series 
! the calculation is being performed and is chosen arbitrarily in the beginning 
! but while in the loop it runs only nstop + 1, nstop, nmx, nn and y are 
! used to guess the number of series in the Mie code only, unneccesary confusion
! in nn, nstop and others
  xstop = x + 4._f * x**0.3333_f + 2.0_f
! xstop is the size parameter upto which we will run the Mie calculations.
  nstop = xstop

  ! Will loop over nang angles which is given as an input to the Mie code.
  ymod = int(abs(y))
  nmx  = max(xstop, ymod) + 15
! Since the electromagnetic wave comes as a plane wave with 180 degree 
! Therefore each angle from 0 to 180 which is pi/2 is analyzed for the 
! particle in the Mie Theory 
  dang = 1.570796327_f / float(nang - 1)
  allocate(d(nmx))
  
! Calulating angles at which radiation will fall over the particles
  do j = 1, nang
...
! Somehow calculating derivative alpha(k) and beta(k) from 
! all the expansion terms of the mie series 
! Search logarithmic in http://www.thermopedia.com/content/137/    
  do n = 1, nn
    rn = nmx - n + 1
...
!*******************************************************************************
! Sum S1 and S2 over all the angles      
    do j = 1, nang
      jj     = 2 * nang-j
      pi(j)  = pi1(j)
