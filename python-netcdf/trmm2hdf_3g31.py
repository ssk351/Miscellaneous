...
sds_1 = hf.select("latentHeating")
latentheating = sds_1.get()
print(latentheating.shape)

sds_2 = hf.select("stratiformFraction")
stratiformfraction = sds_2.get()
print(stratiformfraction.shape)
...
latentheatings        = dataset.createVariable('latentheating', np.float32,('time', 'level','longitude', 'latitude'), fill_value=-9999.9)
stratiformfractions   = dataset.createVariable('stratiformfraction', np.float32,('time', 'longitude', 'latitude'), fill_value=-9999.9)
...
latentheating_1         = np.zeros((1,latentheating.shape[0],latentheating.shape[1],latentheating.shape[2]))
stratiformfraction_1    = np.zeros((1,stratiformfraction.shape[0],stratiformfraction.shape[1]))
...
latentheating_1[:,:,:,:] = -9999.9
stratiformfraction_1[:,:,:] = -9999.9
#alllhmean_1[:,:,:,:] = -9999.9
...
latentheating_1[0,:,:,:]       = latentheating[:,:,:]
stratiformfraction_1[0,:,:]    = stratiformfraction[:,:]
...
latentheatings[:]         = latentheating_1  
stratiformfractions[:]    = stratiformfraction_1 
...
#     float latentHeating ( nlayer, nlon, nlat )
#         units :	K/hr
#         hdf_name :	latentHeating
#         hdf_name :	allPix
#      float stratiformFraction ( nlon, nlat )
#         hdf_name :	stratiformFraction


# Usage
# python trmm2hdf_3g31.py 3G31.20050621.43312.7.HDF test_hdf.nc
