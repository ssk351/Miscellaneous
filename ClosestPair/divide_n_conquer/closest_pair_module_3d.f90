...
contains
  ...
  subroutine closest_pair_simple(p, pair,distance, pos) 
  ...
  subroutine closest_pair(p, pair,distance,pos,n)
    type(point), dimension(:), intent(in) :: p
    type(point), dimension(:), intent(out), optional :: pair
    double precision :: distance
    ...
    call closest_pair_real(xp, yp, dp, distance,pos)
    do i=1,n
      do j=1,n
          if( (p(i)%x.eq.dp(1)%x).and. (p(j)%x.eq.dp(2)%x) .and. (p(i)%y.eq.dp(1)%y).and. (p(j)%y.eq.dp(2)%y).and. (p(i)%z.eq.dp(1)%z) .and. (p(j)%z.eq.dp(2)%z) ) then
            pos = (/ i,j /)
          end if
      end do
    end do
  ...
  recursive subroutine closest_pair_real(xp, yp, pair, distance,pos)
    ...
    double precision :: dl, dr, dmin, xm, d
    integer :: ns, i, k, j, midx
    integer, dimension(2) :: pos 
    if ( size(xp) <= 3 ) then
       call closest_pair_simple(xp, pair, distance,pos)
    else
       ...
       call closest_pair_real(pl, yl, pairl, dl,pos)
       call closest_pair_real(pr, yr, pairr, dr,pos)
